...
export default {
  ...
  run: async (client, m, args) => {
    const text = args[0]
    if (!text) return m.reply('Enter a link to make the request.')

    if (!/^https?:\/\//.test(text))
      return m.reply('Enter a valid link starting with *https://* or *http://*')
    ...
    try {
      ...
      if (/image\/(jpeg|png|gif|webp)/.test(contentType) || ['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(ext)) {
        return await client.sendMessage(m.chat, { image: buffer, caption: `《✧》 Image from: ${text}` }, { quoted: m })
      }

      if (/video\/(mp4|webm|ogg)/.test(contentType) || ['mp4', 'webm', 'ogg'].includes(ext)) {
        return await client.sendMessage(m.chat, { video: buffer, caption: `《✧》 Video from: ${text}` }, { quoted: m })
      }

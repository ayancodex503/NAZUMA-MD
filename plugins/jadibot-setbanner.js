...
export default {
  ...
  run: async (client, m, args) => { 
    ...
    if (
      ...
      return m.reply('✎ You must send or quote an image or video to change the bot banner.');

    if (value.startsWith('http')) {
      config.banner = value;
      return m.reply(`The banner for *${config.namebot2}* has been updated!`);
    }
    ...
    if (!/image\/(png|jpe?g|gif)|video\/mp4/.test(mime))
      return m.reply('✎ Reply to a valid image or video.');

    const media = await q.download();
    if (!media) return m.reply('✎ Could not download the file.');

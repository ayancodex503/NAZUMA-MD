  ...
  run: async (client, m, args) => {
   ...
   const userId = await resolveLidToRealJid(who2, client, m.chat);
    ...
    if (!who2 || !chat.users[userId]) {
      return m.reply('Mention or reply to a valid user to see their warnings.')
    }
    ...
    if (total === 0) {
      return client.reply(m.chat, `@${userId.split('@')[0]} has no warnings registered.`, m, {
        mentions: [userId],
      })
    }

    const name = (global.db.data.users[userId].name) || 'User'
    const warningList = user.warnings
      .map((w, i) => {
        const index = total - i
        const author = w.by ? `\n> » By: @${w.by.split('@')[0]}` : ''
        return `\`#${index}\` » ${w.reason}\n> » Date: ${w.timestamp}${author}`
      })
      .join('\n')

    await client.reply(m.chat,
      `✐ Warnings for @${userId.split('@')[0]} (${name}):\n> ✧ Total warnings: \`${total}\`\n\n${warningList}`,
      m,
